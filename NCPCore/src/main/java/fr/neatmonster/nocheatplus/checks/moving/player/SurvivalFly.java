...
public class SurvivalFly extends Check {
    ...
    /**
     * Holder for mutable allowed distance state used during calculations.
     */
    private static class DistanceState {
        double allowedDistance;
        double friction;
        boolean baseModifiers;
        boolean baseModifiersSprint;
        boolean sneakModifier;
    }
    ...
    */
    ...
    /**
     * Apply environment specific modifiers to the allowed horizontal distance.
     */
    private void applyMediumModifiers(final AllowedDistanceContext ctx,
            ...
            final double modStairs, final double modHopSprint,
        ...
        if (thisMove.from.inWeb) {
            tags.add("hweb");
            state.allowedDistance = Magic.modWeb * thisMove.walkSpeed * cc.survivalFlyWalkingSpeed / 100D;
            state.baseModifiersSprint = false;
            state.baseModifiers = true;
            state.sneakModifier = true;
            state.friction = 0.0;
        }
        ...
        // Powder snow
            ...
            tags.add("hsnow");
            state.allowedDistance = Magic.modPowderSnow * thisMove.walkSpeed * cc.survivalFlyWalkingSpeed / 100D;
        ...
        // Berry bush
            ...
            tags.add("hbush");
            state.allowedDistance = Magic.modBush * thisMove.walkSpeed * cc.survivalFlyWalkingSpeed / 100D;
            if (thisMove.yDistance > data.liftOffEnvelope.getMinJumpGain(data.jumpAmplifier) - Magic.GRAVITY_SPAN
                    && thisMove.from.onGround && !thisMove.to.onGround) {
                state.allowedDistance *= 2.3;
            }
        ...
        // Soulsand
            ...
            tags.add("hsoulsand");
            state.allowedDistance = Magic.modSoulSand * thisMove.walkSpeed * cc.survivalFlyWalkingSpeed / 100D;
            if (BridgeEnchant.hasSoulSpeed(player)) {
                state.allowedDistance *= Magic.modSoulSpeed;
        ...
        // Slimeblock
            ...
            state.allowedDistance = Magic.modSlime * thisMove.walkSpeed * cc.survivalFlyWalkingSpeed / 100D;
            state.sneakModifier = true;
            state.baseModifiers = true;
            state.friction = 0.0;
        }
        ...
        // Honeyblock
            ...
            state.allowedDistance = modHoneyBlock * thisMove.walkSpeed * cc.survivalFlyWalkingSpeed / 100D;
            state.sneakModifier = true;
            state.baseModifiers = true;
            state.friction = 0.0;
        }
        ...
        // Stairs
            ...
            state.baseModifiers = true;
            state.sneakModifier = true;
            state.allowedDistance = modStairs * thisMove.walkSpeed * cc.survivalFlyWalkingSpeed / 100D;
            state.friction = 0.0;
            if (!Double.isInfinite(mcAccess.getHandle().getFasterMovementAmplifier(player))) {
                state.allowedDistance *= 0.88;
            }
        }
        ...
        // NoSlow packet (detection, not a deterministic limit)
            ...
            state.allowedDistance = 0.0;
            state.baseModifiers = false;
            state.friction = 0.0;
        }
        ...
        // Collision tolerance for entities (1.9+)
                ...
                && state.allowedDistance < 0.35 && data.liftOffEnvelope == LiftOffEnvelope.NORMAL) {
            tags.add("hcollision");
            state.allowedDistance = Magic.modCollision * thisMove.walkSpeed * cc.survivalFlyWalkingSpeed / 100D;
            state.baseModifiers = true;
            ...
            if (thisMove.from.inWater || !thisMove.from.inLava) {
                ...
                if (from.isInWaterLogged() && data.insideMediumCount <= 1
                    ...
                    if (Magic.XORonGround(thisMove) || Magic.XORonGround(lastMove)) {
                        state.allowedDistance = thisMove.walkSpeed * data.lastFrictionHorizontal * cc.survivalFlySwimmingSpeed / 100D;
                    }
                }
            }
            ...
            if (BlockProperties.isAir(from.getTypeIdAbove()) && !thisMove.headObstructed && !from.isSubmerged(0.8)
                ...
                if (thisMove.from.inLava) {
                    if (!lastMove.from.inLava || !pastMove2.from.inLava
                        ...
                        state.allowedDistance = (ctx.sprinting() ? Magic.modSprint : 1.0) * thisMove.walkSpeed * cc.survivalFlySwimmingSpeed / 100D;
        ...
        // Speed restriction for leaving a liquid.
            ...
            if (data.surfaceId == 1) state.allowedDistance *= Magic.modSurface[1];
            data.surfaceId = 1;
            ...
            if (blockData > 3 || blockUnderData > 3 || data.isdownstream) {
                data.surfaceId = 0;
                state.allowedDistance = thisMove.walkSpeed * cc.survivalFlySwimmingSpeed / 100D;
                data.isdownstream = false;
            }
        }
        ...
        // Sneaking
            ...
            tags.add("sneaking");
            state.allowedDistance = Magic.modSneak * thisMove.walkSpeed * cc.survivalFlySneakingSpeed / 100D;
            state.allowedDistance += 0.051 * BridgeEnchant.getSwiftSneakLevel(player);
            state.baseModifiers = true;
            state.friction = 0.0;
        ...
        // Using items
            ...
            if (thisMove.from.onGround) {
                if (!thisMove.to.onGround) {
                    ...
                    state.allowedDistance = (lastMove.hDistance > 0.23 ? 0.4 : 0.23 + (ServerIsAtLeast1_13 ? 0.155 : 0.0))
                            + 0.02 * (Double.isInfinite(speedAmplifier) ? 0 : speedAmplifier + 1.0);
                    state.allowedDistance *= cc.survivalFlyBlockingSpeed / 100D;
                    data.noSlowHop = 1;
                }
                else {
                    if (lastMove.toIsValid && lastMove.hDistance > 0.0) {
                        state.allowedDistance = data.noSlowHop < 7
                                ? (lastMove.hAllowedDistance * (0.63 + 0.052 * ++data.noSlowHop))
                                : lastMove.hAllowedDistance;
                    }
                    else {
                        state.allowedDistance = Magic.modBlock * thisMove.walkSpeed * cc.survivalFlyBlockingSpeed / 100D;
                    }
                }
            }
            else if (data.noSlowHop > 0) {
                if (data.noSlowHop == 1 && lastMove.toIsValid) {
                    state.allowedDistance = lastMove.hAllowedDistance * 0.6 * cc.survivalFlyBlockingSpeed / 100D;
                    data.noSlowHop = 4;
                }
                else {
                    state.allowedDistance = lastMove.hAllowedDistance * 0.96 * cc.survivalFlyBlockingSpeed / 100D;
                }
            }
            ...
            state.allowedDistance = Math.max(state.allowedDistance, 0.08);
            state.friction = 0.0;
            state.baseModifiers = false;
            state.baseModifiersSprint = false;
        }
        ...
        // Fallback to the default speed
            ...
            state.baseModifiers = true;
            if (!thisMove.from.onGround && thisMove.to.onGround) {
                state.allowedDistance = Magic.modLanding * thisMove.walkSpeed * cc.survivalFlySprintingSpeed / 100D;
                tags.add("walkspeed_to");
            }
            else if (data.momentumTick > 0) {
                state.allowedDistance = modHopSprint * thisMove.walkSpeed * cc.survivalFlySprintingSpeed / 100D;
                tags.add("walkspeed(" + data.momentumTick + ")");
            }
            else {
                state.allowedDistance = thisMove.walkSpeed * cc.survivalFlySprintingSpeed / 100D;
                tags.add("walkspeed");
            }

            if (!Magic.touchedIce(thisMove)) state.friction = 0.0;
        }
    }
    ...
    private double setAllowedhDist(final AllowedDistanceContext ctx) {
        ...
        state.baseModifiers = false;
        state.baseModifiersSprint = true;
        state.sneakModifier = false;
        if (thisMove.from.onIce) tags.add("hice");

        ////////////////////////////////////////////////////////////////////////
        // Set the allowed horizontal distance according to medium and status //
        ///////////////////////////////////////////////////////////////////////
        applyMediumModifiers(ctx, lastMove, pastMove2, sfDirty, actuallySneaking,
                isBlockingOrUsing, modHoneyBlock, modStairs, modHopSprint, state);

        double hAllowedDistance           = state.allowedDistance;
        double friction                   = state.friction;
        boolean useBaseModifiers          = state.baseModifiers;
        boolean useBaseModifiersSprint    = state.baseModifiersSprint;
        boolean useSneakModifier          = state.sneakModifier;
